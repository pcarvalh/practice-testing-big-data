...
evalInfo_import <- read_csv("SummaryQuizzes_ds490.csv",col_types = cols(`Problem Start Time` = col_datetime(format = "%Y-%m-%d %H:%M:%S"))) ##file available in datashop: https://pslcdatashop.web.cmu.edu/Files?datasetId=490
...
##import log_data
log_act <- read_delim("log_act.txt", "\t", escape_double = FALSE, col_types = cols(server_receipt_time = col_datetime(format = "%Y-%m-%d %H:%M:%S"),session_date = col_skip(), session_server_receipt_date = col_skip(),session_timezone = col_skip(), transaction_time = col_datetime(format = "%Y-%m-%d %H:%M:%S")),trim_ws = TRUE) ##file available in datashop: https://pslcdatashop.web.cmu.edu/Files?datasetId=490

##import names in info
pages <- read_csv("ds_490/countscourse_c_cm2.1_.csv", col_types = cols(X1 = col_skip()))  ##file available in datashop: https://pslcdatashop.web.cmu.edu/Files?datasetId=490

activities <- read_csv("ds_490/activityListcourse_c_cm2.1_.csv", col_types = cols(X1 = col_skip(), purpose = col_skip())) ##file available in datashop: https://pslcdatashop.web.cmu.edu/Files?datasetId=490

info_ids <- c(pages$name,activities$idref)
#rm(pages,activities)

##add student id
log_sess <- read_delim("ds490/log_sess.txt","\t", escape_double = FALSE, trim_ws = TRUE) ##file available in datashop: https://pslcdatashop.web.cmu.edu/Files?datasetId=490

names(log_sess)[c(2,3)] <- c("sess_ref","actual_user_id")

anon_mappings <- read_csv("/datasets/act_log/ds490/ds490_041718.csv")
...
ds <- read_delim("ds490/ds490_tx_All_Data_1586_2017_0319_034545.txt", "\t", escape_double = FALSE, col_types = cols(Time = col_datetime(format = "%Y-%m-%d %H:%M:%S")), trim_ws = TRUE) ##file available in datashop: https://pslcdatashop.web.cmu.edu/Export?datasetId=490
...
ds <- ds[ds$`Problem Name`%in%c(activities$idref),] ##file available in datashop: https://pslcdatashop.web.cmu.edu/Files?datasetId=490
rm(activities)
...
SumData$dataset <- "ds490"
SumData$domain <- "c@cm"

SumDataByNumbers$dataset <- "ds490"
SumDataByNumbers$domain <- "c@cm"

## save data
write.csv(x = SumData,file = "SumData_ds490.csv",row.names = F)
write.csv(x = SumDataByNumbers,file = "SumDataByNumbers_ds490.csv",row.names = F)
